			...
			// YOUR CODE HERE
				...
				while(i >= 0)
				{
					while(j< width)
					{
						...
						int pixcel = *(((int*)(data_arr+0x36))+(j+i*width));

						//Red Color

						short redColor = (pixcel >> 16)&0xFF;  // we need red color starting at 0 til 4
						redColor = redColor/8; // scale down to 8-bit values.

						//greenColor
						short greenColor = ((pixcel >> 8)&0xFF);
						...
						short blueColor = (pixcel&0xFF);
						blueColor = blueColor/8;
						blueColor = (blueColor<<10);

						// print format : print anywhere like even in file..
						fprintf(outfile, "0x%x, ", (blueColor|greenColor|redColor));
